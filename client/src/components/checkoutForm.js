...
function CheckoutForm() {
    ...
    async function handlePay(e) {
        ...
        setPaying(true);
        const { client_secret } = await fetch(`${process.env.REACT_APP_API_URL}/create-payment`, {
            method: "POST",
            headers: {
        ...
        const { paymentIntent, error } = await stripe.confirmCardPayment(client_secret, {
            payment_method: {
                ...
                billing_details: {
                  name: 'John Doe',             // Include the customer's name here
                  address: {
                    line1: '123 Main Street',   // Include the customer's address here
                    city: 'City Name',
